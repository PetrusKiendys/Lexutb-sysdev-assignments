...
namespace miniproject1 {
    ...
    public class LaptopComputer : PortableComputer {
        ...
        public LaptopComputer(string name, DateTime purchaseDate, double price, Location location, string brand, int weight, string cpu, string gpu, string ram, int ramSize, string storage, int storageSize, string displayType, double displaySize, string batteryType, int batteryCapacity, bool trackpad, bool webcam)
        : base(name, purchaseDate, price, location, brand, weight, cpu, gpu, ram, ramSize, storage, storageSize, displayType, displaySize, batteryType, batteryCapacity) {
    ...
    public abstract class Asset {
        ...
        internal Asset(string name, DateTime purchaseDate, double price, Location location) {
            ...
            // string localPriceTag = getLocalPriceTag(price, location);    // EXTRA: implement function that returns formatted "local price tag" string,
                                                                            //        based on (1) price of asset in USD, (2) exchange rate and (3) location of asset
                                                                            //        this would substitute and remove the need of props 'prettyPrice' and 'currency'
        }

        internal double getLocalPrice(double price, Location location) {
